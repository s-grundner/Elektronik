...
In der Shockleygleichung haben

- der Sperrstrom $I_{S} = I_{S}(T)$ und
- die [Temperaturspannung](Temperaturspannung.md) $U_{\mathrm{T}} = U_{\mathrm{T}}(T)$

eine Relevante Temperaturabhängigkeit.


$$
I_{\mathrm{D}}(U_{\mathrm{D}}, T) = I_{\mathrm{S}}(T) \left(  e^{ \tfrac{U_{\mathrm{D}}}{U_{\mathrm{T}}(T)} } - 1\right) 
$$

mit 

$$
I_{\mathrm{S}}(T) = I_{\mathrm{S},0} e^{\tfrac{ -W_{\mathrm{G}} }{kT}} \qquad U_{\mathrm{T}}(T) = \frac{kT}{e}
$$

Umgeformt auf $U_{\mathrm{D}}$:

$$
U_{\mathrm{D}}(T) = U_{\mathrm{T}}(T)\ln \left( \frac{I_{\mathrm{D}}}{I_{\mathrm{S}}(T)}+1 \right)
$$

## Empfindlichkeit

$$
\frac{ \partial U_{\mathrm{D}} }{ \partial T } \bigg|_{I_{\mathrm{D}}=\text{konst.}}  
$$

## Kleinsignalbetrieb

### Taylorreihe - Dynamischer Leitwert
...
$$
I_{D}(U) \approx \underbrace{I_{\mathrm{S}}\left(\mathrm{e}^{\frac{U_0}{n U_{\mathrm{T}}}}-1\right)}_{I_0}+\underbrace{\frac{I_{\mathrm{S}}}{n U_{T}}e^{\frac{U_{0}}{nU_{T}}}}_{\frac{\mathrm{d} I}{\mathrm{~d} U}}\left(U-U_0\right)+\underbrace{\frac{I_{\mathrm{S}} }{2 n^2 U_{\mathrm{T}}^2}}_{\frac{1}{2} \frac{\mathrm{~d}^2 I}{\mathrm{~d} U^2}}\left(U-U_0\right)^2
$$
...
$$
I_{\mathrm{D}}(U) \approx I_{0} + (U-U_{0})\left.G_{d}\right|_{U_{0}} + \frac{(U-U_{0})^{2}}{2}\left.\frac{\mathrm{d}G_{d}}{\mathrm{d}U}\right|_{U_{0}}
$$

Mit $U(t)=U_{0}+u(t)$ als Spannung die um einen DC Offset schwingt ergibt sich die **Kleinsignalannäherung**:
...
- Gleichrichter
- Elektronisch steuerbarere Spannungsteiler für kleine Wechselspannungen