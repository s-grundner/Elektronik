...
:LiRefreshCcw: [Erreichbarkeit Zeitkontinuierlicher LTI-Systeme](Zeitkontinuierlich/Erreichbarkeit.md)

---

> [!def] **D1 - ERRE)** Erreichbarkeit eines zeitdiskreten Systems ^ERRE
> Man nennt ein [zeitdiskretes LTI-System](LTI-Zustandsraum.md#^LTI-MIMO) vollständig erreichbar, wenn ausgehend vom Anfangszustand $\mathbf{x}_{0} = 0$ jeder beliebige Zustand $\mathbf{x}_{N}$ mit einer endlichen Steuerfolge $\mathbf{u}_{k} = (\mathbf{u}_{1}, \mathbf{u}_{2},\dots,\mathbf{u}_{N-1})$ erreicht werden kann.

## Testen Auf Erreichbarkeit

Der Test auf Erreichbarkeit erfolgt gleich wie für zeitkontinuierliche Systeme.

![Testen Auf Erreichbarkeit](Zeitkontinuierlich/Erreichbarkeit.md#Testen%20Auf%20Erreichbarkeit)

## Erreichbarer Unterraum

![Erreichbarer Unterraum](Zeitkontinuierlich/Erreichbarkeit.md#^ERRE-SUBSPACE)
...
Das neue System hat die Form (mit den Koeffizienten wie in [ZTRF](../Zeitkontinuierlich/Zustandstransformation.md#^ZTRF))

![](Regelungstechnik/Kalman-Zerlegung.md#^ERRE-DISK)

## Steuerbarkeit

> [!question] :LiRefreshCcw: [Steuerbarkeit Zeitkontinuierlicher Systeme](Zeitkontinuierlich/Erreichbarkeit.md#Steuerbarkeit)

---

> [!def] **D)** Steuerbarkeit
> Das [zeitdiskrete LTI-System](LTI-Zustandsraum.md#^LTI-MIMO) vollständig steuerbar, wenn ausgehend von einem beliebigen Anfangszustand $\mathbf{x}_{0}$, mit einer endlichen Steuerfolge $\mathbf{u}_{k} = (u_{0},u_{1},\dots, u_{N-1})$ das System in den Zustand $\mathbf{x}_{N} = \mathbf{0}$ übergeführt werden kann. 
...
> [!question] [Stabilisierbarkeit Zeitkontinuierlicher LTI-Systeme](Zeitkontinuierlich/Erreichbarkeit.md#Stabilisierbarkeit)

---

Das nicht vollständig beobachtbare System ist **stabilisierbar**, wenn $\mathbf{\bar{A}}_{22}$ eine [Einheitskreismatrix](../../Mathematik/Algebra/Einheitskreismatrix.md) ist.