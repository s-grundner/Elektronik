---
tags:
  - Baugruppe/OPV
  - Baugruppe/Verstärker
aliases:
  - OPV
  - "#Baugruppe/OPV"
keywords:
subject:
  - VL
  - Technische Elektronik
  - Einführung Elektronik
semester: SS25
created: 28. Februar 2025
professor:
  - Timm Ostermann
release: false
title: Operations-Verstärker
---
 

# Operations-Verstärker

## Grundschaltungen

> [!important] Wichtig bei Grundschaltungen von Invertierenden Verstärkern:
> - Die Eingangsspannung $U_e$ wird immer invertiert, d.h. das Vorzeichen wird umgekehrt.
> - Das Bauteil am Eingang Prägt den Strom in das Bauteil der Rückkopplung ein 

```dataview
TABLE FROM #Baugruppe/OPV
```



> [!important] [Reales Verhalten von OPVs](Reale%20Eigenschaften%20von%20OPVs.md)

## Realer Aufbau

Der OPV ist intern aus meistens drei Blöcken aufgebaut:

1. Eingangsdifferenzenverstärker
	- hoher Eingangswiderstand
	- setzt Differenzenspannug in einen proportionalen Strom um
2. Verstärkerstufe
	- Setzt den Strom in eine hohe Spannung um
3. Ausgangsstufe
	- niederohmiger Ausgang
	- keine Spannungsverstärkung

### Realer Aufbau in mit MOSFETs

> [!question] [MOSFET](Halbleiter/Metall-Oxid-Halbleiter-Feldeffekttransistor.md)



# Quellen

- [uni-muenster.de/Physik.AP/Veranstaltungen/F-Praktikum/anleitungen/Operationsverstaerker.pdf](https://www.uni-muenster.de/Physik.AP/Veranstaltungen/F-Praktikum/anleitungen/Operationsverstaerker.pdf)
- [HTL OPV](../_assets/pdf/OPV.pdf)
- [HTL OPV Grundschaltungen](../_assets/pdf/OPV%20Grundschaltungen.pdf)
