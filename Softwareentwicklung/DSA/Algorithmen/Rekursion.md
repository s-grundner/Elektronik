...
| Can Reduce [time complexity](Algorithmen/O-Notation.md) easily with [Memoisation](Rekursion.md#Memoisation) | Can be unnecessarily complex if poorly constructed |
...
To reduce [time complexity](Algorithmen/O-Notation.md), it is recommended to save repeating patterns (in objects or structs).

> [!EXAMPLE] fibonacci  
> non-optimized function, without memoization:  
> [time complexity](Algorithmen/O-Notation.md): $O(2^{n})$❌  
> [space complexity](Algorithmen/O-Notation.md): $O(n)$ ✅
>
> ``` js
> const fib = (n) => {
> 	if(n <= 2) return 1;
...
> [time complexity](Algorithmen/O-Notation.md): $O(n)$ ✅  
> [space complexity](Algorithmen/O-Notation.md): $O(n)$ ✅
>
> ```js
> const fib = (n, memo = {}) => {
