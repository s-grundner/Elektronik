...
> $$
> 
> Dann besitzt das [AWP](index.md) im Intervall $[t_{0},t_{0}+\alpha]$ eine **eindeutige**, stetig differenzierbare Lösung $x$ (mit $\alpha$ aus [Peano](Peano.md)).
> 
> **(b)** Weiters gilt, dass die [Funktionenfolge](Funktionenfolge.md):
> $$
> \varphi_{n+1}(t):=x_0+\int_{t_0}^t f\left(\tau, \varphi_n(\tau)\right) d \tau, \quad \varphi_0(t):=x_0,
> $$
> 
> gleichmäßig auf $\left[t_0, t_0+\alpha\right]$ für $n \rightarrow \infty$ gegen die Lösung $x$ [konvergiert](Kurvendiskussion/Grenzwert.md). Zusätzlich gilt auf diesem Intervall für alle $n \in \mathbb{N}_0$ die Abschätzung
> 
> $$
...
Die Funktionenfolge aus **(b)** ist die [sukzessive Approximation](sukzessive%20Approximation.md) einer Differentialgleichung.
