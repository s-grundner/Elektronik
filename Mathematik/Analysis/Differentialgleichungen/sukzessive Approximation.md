...
> [!important] Es wird ein [AWP](Differentialgleichungen/index.md)  $\dot{x} = f(t,x)\quad x(t_{0})=x_{0}$ betrachtet.
> 
...
> $$
> 
> 3. Die Lösung $x$ des [AWP](Differentialgleichungen/index.md) wird [analytisch](../Numerische%20Verfahren/Numerische%20Lösungsverfahren%20von%20DGL.md) mit der Funktionenfolge $\varphi_{n}$ approximiert:
> 
...
Für eine Lösung $x$ von dem [AWP](Differentialgleichungen/index.md) bzw einen Fixpunkt $x$ von $A$ gilt nach [Lindelöf](Differentialgleichungen/Picard-Lindelöf.md):
...
$$
\begin{align}
\Phi_{n}(t) = \mathbf{I}_{m}+ \int_{t_{0}}^{t}A(s)\cdot\Phi_{n-1}(s)\,\mathrm{d}s, \quad\Phi_{0}:= \mathbf{I}_{m} \\
