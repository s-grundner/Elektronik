...
$$
e^{ tA } = e^{ \lambda t \mathbf{I} +t(A-\lambda \mathbf{I}) } = e^{ \lambda t \mathbf{I} } \cdot e^{ t(A-\lambda \mathbf{I}) } = e^{ \lambda t \mathbf{I} } \cdot \sum_{k=0}^{\infty}\frac{t^k}{k!} (A-\lambda \mathbf{I})^k
$$

> [!info] Zum Exponenten wird $+t\lambda \mathbf{I}-t\lambda \mathbf{I}$ hinzugerechnet.
> $e^{t\mathbf{A}}=e^{ t\mathbf{A}+t\lambda \mathbf{I}-t\lambda \mathbf{I} }=$

> [!important] Durch Umformung erhalten wir den bekannten Ausdruck $A-\lambda \mathbf{I}$ zu den [Eigenvektoren](../../Algebra/Eigenvektor.md): 
...
> $$
> \begin{align}
> e^{ \lambda t \mathbf{I}} \cdot \sum_{k=0}^{\infty}\frac{t^k}{k!} (A-\lambda \mathbf{I})^k \cdot v = e^{ \lambda t\cdot \mathbf{I} } \cdot v 
> \end{align}
> $$
> Falls $v$ ein Eigenvektor ist, dann gilt $(A-\lambda \mathbf{I})\cdot v=0$. 


> [!success] Die Summe bricht nach $k=0$ ab da für die restlichen Glieder $(A-\lambda \mathbf{I})\cdot v=0$ gilt. 
> Es zählt nur der erste Summand da $0^{0}$ als $\mathbf{I}$ definiert ist.
