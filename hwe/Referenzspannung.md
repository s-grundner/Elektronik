...
Man nutzt die **Bandabstand-Spannung** eines [[Halbleiter]] Bauelements zur Erzeugung einer Hochgenauen Referenzspannung $~1{,}2V$
### Grundlagen
1. [Bandabstand](https://de.wikipedia.org/wiki/B%C3%A4ndermodell)
[[Halbleiter|Halbleitermaterialien]] weisen einen Abstand in Elektronen Volt $eV$ zwischen Valenzband und Leitungsband auf.
$U_{g}= \frac{W_{g}}{e}$
wobei $e$ die [[Konstanten Verzeichnis#Elementar Ladung\|Elementar Ladung]] ist.

>![[Baendermodell.png]]
	
2. [[Shockley Gleichung]]
...
| ------------- | ---------------------------------------------- | ----------------------------------------------------------------------- |
| $\eta$        | [[{MOC} Transistor\|Transistorgeometrie]] $(\eta=1)$ |                                                                         | 
| $U_{T}$       | Temperaturspannung                             | $U_{T}=\frac{k\cdot T}{e}\simeq 25mV$ <br> $k\cdots$ [[Konstanten Verzeichnis\|Boltzmannkonstante]] |
...
Mit der folgenden [[Kirchhoffsche Regeln#Maschen Regel|Masche]] lässt sich auf die Spannung über die Kollektorwiderstände ermitteln:
...
|    2    | $$\begin{align*}U_{BE2} &= U_{BE1}+\Delta U_{BE}\\\Delta U_{BE}&= U_{BE2}-U_{BE1}\\&= U_{T}\cdot(\ln(\frac{I_{C2}}{I_{S}})-\ln(\frac{I_{C1}}{I_{S}}))\\&= U_{T}\cdot(\ln(\frac{I_{C2}}{I_{S}}\cdot\frac{I_{S}}{I_{C1}}))\\&= \underbrace{U_{T}}_{\frac{k\cdot T}{e}}\cdot\ln(\frac{I_{C2}}{I_{C1}})\\\frac{k\cdot T}{e}&\propto T\end{align*}$$   | Idee:<br>Die Spannung $U_{BE}$ hat mit $-2 \frac{mV}{K}$ einen Negativen Temperaturkoeffizienten $\Delta U_{BE}$<br>und damit auch die Spannung $U_{PTAT}$<br>(über $I_{C1}$, $I_{C2}$), besitzen einen **positiven [[Temperatur und Teilchenmodell\|Temperatur]]koeffizienten**.<br>Durch **Summation der Spannungen** ($U_{ref}=\Delta U_{BE}+U_{PTAT}$) ergibt sich bei entsprechender Dimensionierung<br>eine **Kompensation** der [[Temperatur und Teilchenmodell\|Temperatur]]koeffizienten. |
